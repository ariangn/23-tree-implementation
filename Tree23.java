...
class Tree23 extends Util {
	...
	InternalNode find (String x) {
		...
		while (!(u.child[0] instanceof LeafNode)) {
			// if new u is first child
			if (x.compareTo(u.child[0].guide) <= 0) {
				u = (InternalNode) u.child[0];
			}
			// if new u is second child
			else if (x.compareTo(u.child[1].guide) <= 0) {
				u = (InternalNode) u.child[1];
			}
			// if u is bigger than second child but third child doesn't exist 
			else if (x.compareTo(u.child[1].guide) > 0 && u.degree() == 2) {
				u = (InternalNode) u.child[1];
			}
			// if new u is third child
			else {
	...
	LeafNode search (String x) {
		...
		// iterate through children until x is found
	...
	boolean insert (Item it) {
		...
		int insCh = u.addLeaf(it); // returns # of children of u after insertion
		// 4 children, need to split
		if (insCh == 4) {
			split(u);
			newGuides(u);
			return true;
	...
	Item delete (String x) {
		...
		// find leaf to delete
		LeafNode leaf = search(x);
		if (leaf == null) {
			return null;
		}
		Item deleted = u.removeLeaf(x);
		// check if parent has degree 1
		if (u.degree() <= 1) {
			repairTree(leaf.parent);
		}
		for (int i=0; i<u.parent.degree(); i++) {
			newGuides((InternalNode) u.parent.child[i]);
		}
		if (deleted != null) {
			return deleted;
		}
	...
	void repairTree(InternalNode u) {
		boolean proposal = false;
		// special case for removing last nodes
		if (ht == 1) {
			if (u.degree() == 0) {
				ht--;
			}
		...
		// check if u is degree 1
		if (u.degree() == 1) {
			// check if at root, if so decrease height
			if (root == u) {
				if (ht > 1) {
			...
			// check if u has left sibling
			int uIndex = u.parent.getIndexOf(u);
			if (uIndex == 0) {
				proposal = true;
			}
			if (proposal == false) {
				// propose left if so 
				u.parent.proposeLeft(uIndex);
			}
			// else propose right
			else {
				u.parent.proposeRight(uIndex);
			}
		// call parent recursively
		if (u.parent.degree() == 1) {
			repairTree(u.parent);
		}
		}
	}

	void split (InternalNode u) {
		// check if splitting root, create new root if so
		if (u.parent == null) {
			InternalNode newrt = new InternalNode();
			newrt.child[0] = u;
			u.parent = newrt;
			newrt.guide = u.guide;
			this.root = newrt;
		...
		// create new left sibling, give them first two children
		InternalNode newsib = new InternalNode();
		newsib.newParent(u.child[0],u.child[1],newsib);
		int oldindex = u.parent.getIndexOf(u);
		u.parent.shiftRight(oldindex);
		newsib.parent = u.parent;
		u.parent.child[oldindex] = newsib;
		newsib.guide = u.child[1].guide;

		// shift left twice for orig/right sibling
		u.shiftLeft(0);
		u.shiftLeft(0);
		
		// if parent needs splitting, recursively call split
		if (u.parent.degree() == 4) {
			split(u.parent);
		}
	}

	void newGuides(InternalNode u) {
		if (root.child[0] == null) {
			root = new InternalNode();
			return;
		}
		int maxIndex = u.degree();
		if (u.guide.compareTo(u.child[maxIndex - 1].guide) != 0) {
			u.guide = u.child[maxIndex - 1].guide;
		}
		if (u.parent != null) {
			newGuides(u.parent);
		}
